...
El algoritmo de búsqueda lineal funciona de la siguiente manera. Itera sobre el arreglo de izquierda a derecha, buscando un elemento específico.
...
Nueve es lo que estamos buscando, y este elemento del arreglo, su valor es nueve. Y así encontramos lo que buscábamos y podemos detenernos. La búsqueda lineal ha sido completada con éxito.
...
Pero podemos seguir repitiendo este proceso, examinando cada elemento, esperando encontrar el número 50. Pero no sabremos si hemos encontrado o no el número 50, hasta que hayamos revisado cada elemento del arreglo.

Solo una vez que lo hayamos hecho y no hayamos encontrado nada, podemos concluir que el 50 no está en el arreglo. Y así, el algoritmo de búsqueda lineal no falla, per se. Sino que no encontró el 50, pero el 50 no estaba en el arreglo. Pero hemos buscado exhaustivamente cada elemento y, aunque no hayamos encontrado nada, la búsqueda lineal aún tiene éxito incluso si el elemento no está en el arreglo.

Entonces, ¿cuál es el peor escenario con la búsqueda lineal? Bueno, tenemos que revisar cada elemento, ya sea porque el elemento objetivo es el último del arreglo o porque el elemento que buscamos en realidad no existe en el arreglo. ¿Cuál es el mejor escenario? Bueno, podríamos encontrar el elemento de inmediato. ¿Y cuántos elementos tenemos que mirar en el mejor de los casos, si lo estamos buscando y lo encontramos al principio? Podemos detenernos de inmediato.
...
Soy Julia Gonzalez. Esto es CS50.
"""
